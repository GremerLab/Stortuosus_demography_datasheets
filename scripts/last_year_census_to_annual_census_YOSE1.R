...
last.year.census <- read_excel("C:/Users/jrgremer/Box/Gremer Lab/Streptanthus project/Demography/Yosemite/Yose 1/Annual Census 2023/YOSE1_annualcensus_2023_finished.xlsx")
...
#for specific order of transects
transect_order = c("Y1.1", "ST")
...
write.csv(fatesdatasheet,"C:/Users/jrgremer/Box/Gremer Lab/Streptanthus project/Demography/Blank demog datasheets/Annual census data sheets/YOSE1_annualcensus_2024_prefilled_notformatted.csv", row.names = FALSE)


#create datasheet for new data at the quad level, which we are taking for 2024 (limited sampling)
trans1 = expand.grid(transect = "Y1.1", quad = seq(0,5-0.5,0.5)) #transect 1 is 10m
trans2 = expand.grid(transect = "ST", quad = seq(0,5-0.5,0.5))  #transect 2 is 5m


newdatasheet = rbind.data.frame(trans1, trans2) %>%
               mutate(num_V = "", num_B = "", num_F = "", num_P= "", notes = "")

write.csv(newdatasheet,"C:/Users/jrgremer/Box/Gremer Lab/Streptanthus project/Demography/Blank demog datasheets/Annual census data sheets/YOSE1_annualcensus_2024_quadlevel_notformatted.csv", row.names = FALSE)

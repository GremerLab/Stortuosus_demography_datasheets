...
last.year.census <- read_excel("C:/Users/jrgremer/Box/Gremer Lab/Streptanthus project/Demography/Wrights Lake_Carson Pass/Annual Census - WL & CP/Annual Census 2023/WL2_annualcensus_2023_finished.xlsx")
...
write.csv(fatesdatasheet,"C:/Users/jrgremer/Box/Gremer Lab/Streptanthus project/Demography/Blank demog datasheets/Annual census data sheets/WL2_annualcensus_2024_prefilled_notformatted.csv", row.names = FALSE)


#create datasheet for new data at the quad level, which we are taking for 2024 (limited sampling)
trans1 = expand.grid(transect = c("201A", "201B", "201C", "201D"), quad = seq(0,5-0.5,0.5)) #transect 1 is 10m
trans2 = expand.grid(transect = c("202A", "202B", "202C", "202D"), quad = seq(0,5,0.5))  #transect 2 is 5m
trans3 = expand.grid(transect = c("203A", "203B", "203C", "203D"), quad = seq(0,5-0.5,0.5))  #transect 2 is 5m
trans4 = expand.grid(transect = c("204A", "204B", "204C", "204D"), quad = seq(0,5-0.5,0.5))  #transect 2 is 5m


newdatasheet = rbind.data.frame(trans1, trans2, trans3, trans4) %>%
               mutate(num_V = "", num_B = "", num_F = "", num_P= "", notes = "") %>%
               arrange(transect, quad)
              

write.csv(newdatasheet,"C:/Users/jrgremer/Box/Gremer Lab/Streptanthus project/Demography/Blank demog datasheets/Annual census data sheets/WL2_annualcensus_2024_quadlevel_notformatted.csv", row.names = FALSE)

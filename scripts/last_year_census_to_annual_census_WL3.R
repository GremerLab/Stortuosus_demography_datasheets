...
last.year.census <- read_excel("C:/Users/jrgremer/Box/Gremer Lab/Streptanthus project/Demography/Wrights Lake_Carson Pass/Annual Census - WL & CP/Annual Census 2023/WL3_annualcensus_2023_finished.xlsx")
...
write.csv(fatesdatasheet,"C:/Users/jrgremer/Box/Gremer Lab/Streptanthus project/Demography/Blank demog datasheets/Annual census data sheets/WL3_annualcensus_2024_prefilled_notformatted.csv", row.names = FALSE)


#create datasheet for new data at the quad level, which we are taking for 2024 (limited sampling)
trans1 = expand.grid(transect = "1", quad = seq(0,10-0.5,0.5)) #transect 1 is 10m
trans2 = expand.grid(transect = "2", quad = seq(0,5-0.5,0.5))  #transect 2 is 5m
trans3 = expand.grid(transect = "3", quad = seq(0,10-0.5,0.5))  #transect 2 is 5m
trans4 = expand.grid(transect = "4", quad = seq(0,10-0.5,0.5))  #transect 2 is 5m


newdatasheet = rbind.data.frame(trans1, trans2, trans3) %>%
               mutate(num_V = "", num_B = "", num_F = "", num_P= "", notes = "")

write.csv(newdatasheet,"C:/Users/jrgremer/Box/Gremer Lab/Streptanthus project/Demography/Blank demog datasheets/Annual census data sheets/WL3_annualcensus_2024_quadlevel_notformatted.csv", row.names = FALSE)
